...
public class OpenLDAP {
	...
	public static void Login(OpenLDAPConfig ldapConfig,String username,String password) throws Exception {
        ...
        try {
            ...
            try {
            	authentication(userContext, ldapConfig.getSearchBase(), ldapConfig.getPROVIDER_URL(),
            			username, password);
            ...
            throw new ClientVisibleException(
                    "Unable to authenticate service account.", 
                    "Unable to create context with service account credentials." +
                    "Username(DN):" + ldapConfig.SECURITY_PRINCIPAL );
        }finally {
            if (userContext != null) {
                try {
                    userContext.close();
	...
	public static LdapContext authentication(DirContext ctx, String ldapSearchBase, String ldapAdServer, String username,
			String password) throws NamingException,Exception {
		SearchResult srLdapUser = findAccountByAccountName(ctx, ldapSearchBase, username);
		if (srLdapUser==null)
			throw     new ClientVisibleException(
                    "Unable to authenticate service account.", "no such user");
